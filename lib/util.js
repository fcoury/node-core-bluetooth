...
module.exports = {
  ...
  to$Uuid: function(uuid_) {
    uuid_ = uuid_.replace(/-/g, '');
    if (uuid_.length > 4) {
      uuid_ = uuid.unparse(new Buffer(uuid_, 'hex'));
    }

    return $.CBUUID('alloc')('initWithString', $(uuid_)); // private API
  },
  ...
  $arrayToArray: function($array, factory=(obj) => obj) {
    const result = [];
