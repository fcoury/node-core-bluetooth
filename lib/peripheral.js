...
function Peripheral($peripheral) {
  ...
  this.address     = uuidToAddress(this.identifier);
...
Peripheral.prototype.discoverCharacteristicsForService = function(characteristicUUIDs, service) {
  const uuids = characteristicUUIDs && $util.to$Uuids(characteristicUUIDs);
  this.$('discoverCharacteristics', uuids, 'forService', service.$);
};
