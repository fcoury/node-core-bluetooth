...
CentralManager.prototype.retrieveConnectedPeripherals = function(services, allowDuplicates) {
  var $services = $util.to$Uuids(services);
  var $peripherals = this.$('retrieveConnectedPeripheralsWithServices', $services);
  var peripherals = $util.$arrayToArray($peripherals, p => new Peripheral(p));
  this.delegate.addPeripherals(peripherals);
  return peripherals;
};
